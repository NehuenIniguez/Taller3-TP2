...
public class Movimiento : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector2.Dot(swipe, Vector2.up) > 0.7f)
            ...
            transform.position += Vector3.up * movimientoDistancia * Time.deltaTime  ;
            }
        else if (Vector2.Dot(swipe, Vector2.down) > 0.7f)
            {
            Debug.Log("Swipe hacia abajo");
            transform.position += Vector3.down * movimientoDistancia * Time.deltaTime ;
            }
        else if (Vector2.Dot(swipe, Vector2.left) > 0.7f)
            {
            Debug.Log("Swipe a la izquierda");
            transform.position += Vector3.left * movimientoDistancia * Time.deltaTime ;
            }
        else if (Vector2.Dot(swipe, Vector2.right) > 0.7f)
            {
            Debug.Log("Swipe a la derecha");
            transform.position += Vector3.right * movimientoDistancia * Time.deltaTime ;
            }
    }
